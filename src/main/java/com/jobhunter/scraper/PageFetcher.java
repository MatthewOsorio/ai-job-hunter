  ...
  private FetchResult fetchWithJsoup(String url) {
    try {
      Document doc = Jsoup.connect(url).userAgent(USER_AGENT)
          ...
          .header("Accept-Language", "en-US,en;q=0.9").followRedirects(true).timeout(30_000).get();
  ...
  private FetchResult fetchWithPlaywright(String url, Browser browser) {
    try {
      BrowserContext context = browser.newContext(
          new Browser.NewContextOptions().setUserAgent(USER_AGENT).setViewportSize(1920, 1080));
